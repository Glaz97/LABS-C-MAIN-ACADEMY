...
namespace Final_X_Project.TelegramBot.Commands
{
    public class BotCommands
    {
        ...
        public void SendTelegramMessage(int userId, Orders order, BotMessageTypes.TypeOfMessage typeOfMessage)
        {
            ...
            {
                SendAddMessage(GetUserDataById(userId), GetPizzaById(order.PizzaID), order);
            }
            else if (typeOfMessage == BotMessageTypes.TypeOfMessage.Edit)
            {
                SendEditMessage(GetUserDataById(userId), GetPizzaById(order.PizzaID), order);
            }
            else if (typeOfMessage == BotMessageTypes.TypeOfMessage.Custom)
            {
                SendCustomMessage(GetUserDataById(userId), GetPizzaById(order.PizzaID), order);
            }
        }

        public async void SendAddMessage(UsersContactData userContactData, PizzasNomenclature pizza, Orders order)
        {
        ...
        public async void SendEditMessage(UsersContactData userContactData, PizzasNomenclature pizza, Orders order)
        {
            bot_client = new TelegramBotClient(botSettings.GetKey());
        ...
        public async void SendCustomMessage(UsersContactData userContactData, PizzasNomenclature pizza, Orders order)
        {
            bot_client = new TelegramBotClient(botSettings.GetKey());
        ...
        public UsersContactData GetUserDataById (int userId)
        {
            return db.UsersContactData.Where(x => x.UserID == userId).Select(x => x).FirstOrDefault();
        }
